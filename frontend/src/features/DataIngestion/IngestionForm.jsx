...
export const IngestionForm = () => {
  ...
  const { progress, status, error } = useTaskPolling(activeTaskId, {
    onComplete: async (result) => {
      ...
      try {
        const info = await marketApi.getVbtInfo(fetchParams.symbol.toUpperCase());
        if (info && info.indicators) {
          setCalculatedIndicators(fetchParams.symbol.toUpperCase(), info.indicators);
        }
      } catch (e) {
        console.error(e);
      }
  ...
  const handleIngest = async (e) => {
    ...
    try {
      const data = await marketApi.executeVbtFetch(fetchParams);
      ...
      <form onSubmit={handleIngest} className="p-6 space-y-5 flex-1">
        {message && (
          <div className={`p-3 rounded text-xs border font-mono ${
            ...
            {message.text}
            {activeTaskId && <div className="mt-1 font-bold">Progression : {progress}%</div>}
          </div>
        )}
