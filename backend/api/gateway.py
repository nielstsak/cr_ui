...
class ZMQBridgeServer(threading.Thread):
    ...
    def run(self):
        ...
        logger.info(f"ZMQBridgeServer actif sur tcp://{self.host}:{self.port}")
    ...
    def _load(self):
        if os.path.exists(self.file_path):
            try:
                ...
                logger.error(f"Erreur de chargement d'état : {e}")
...
def run_async_background_task(coro_func, *args, **kwargs) -> str:
    ...
    async def wrapper():
        try:
            ...
            logger.error(f"Erreur d'arrière-plan sur {task_id}: {e}")
...
async def _mtf_ingestion_coro(task_id: str, symbol: str, timeframes: List[str], days_history: int):
    ...
    try:
        ...
        logger.error(f"Échec critique de l'acquisition MTF : {str(e)}")
        raise e
    finally:
...
async def get_symbol_descriptive_stats(symbol: str):
    """
    Analyse descriptive et vectorielle d'une paire d'actifs sur l'ensemble de ses
    unités de temps disponibles sur disque. Calcule les klines, min, max, hold ratio,
    les volumes moyens et les profils moyens de mèches hautes/basses.
    """
    symbol = symbol.upper().replace("/", "")
    base_dir = os.path.join(app_state.state["configurations"]["storage_dir"], "BINANCE", symbol)
    
    if not os.path.exists(base_dir):
        raise HTTPException(status_code=404, detail=f"Symbole {symbol} inexistant ou non encore téléchargé.")
